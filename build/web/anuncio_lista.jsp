        ...
        <div id="main_content">
            ...
            <div class="main_perfil">
              ...
              <%
                    ...
                    List<Anuncio> anunc = AnuncioDao.buscar((Integer)session.getAttribute("id"));
              ...
              <div class="main_perfil_conteiner"> 
                    <div class="imagen_productosuser"> <img width="30px" height="30px" src="<%= request.getServletContext().getContextPath() + "/ProductoMultimediaServlet?id=" + anuncio.getIdProducto()+ "&col=1" %>" />  
                       
                    </div>
                   <div class="detalles_contenido">
                       <div class="nombre_detalle_contenido">
                           <label for="nombre_producto"><a><%= anuncio.getNombreProducto()%></a></label>
                       </div>
                       <div class="info_detalle_contenido">
                           <label id="subcategoria"><%= anuncio.getSubcategoria()%></label><br>
                           Fecha de Publicacion:<label id="subcategoria"><%= anuncio.getFechPublicacion() %></label>  Vigencia:<label id="subcategoria"><%= anuncio.getVigencia()%></label><br>
                           <label id="existencia">Productos en existencia:</label><label id="existencia"><%= anuncio.getExistencia()%></label><br>
                       </div>
                      
                        <div class="prod_price_big"><label id="precio">$<%= anuncio.getPrecio()%></label></div>
                        <div class="button5_pos"><form action="AnuncioServlet?accion=editar&id=<%= anuncio.getId()%>" method ="post"><input type="submit" id="button2" value="Editar"/></form> </div>
                        <div class="button6_pos"><form action="AnuncioServlet?accion=borrar&id=<%= anuncio.getId()%>" method ="post"><input type="submit" id="button3"value="Eliminar"/></form> </div>
