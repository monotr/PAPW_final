    ...
    <body onload='started();'>
        ...
        <div id="main_content">
            ...
            <div class="main_perfil">
                <div class="button4_pos"><form action="producto_edit.jsp" method ="post">
                            <input type="submit" value="Nuevo" id="button4"/></form></div>
            <%
                    int idus = (Integer)(session.getAttribute("id"));
                    List<Producto> product = ProductoDao.buscar(idus);
            ...
            %>
              <div class="main_perfil_conteiner"> 
                    <div class="imagen_productosuser"> <img width="30px" height="30px" src="<%= request.getServletContext().getContextPath() + "/ProductoMultimediaServlet?id=" + producto.getId() + "&col=1" %>" /></div>
                   <div class="detalles_contenido">
                       <div class="nombre_detalle_contenido">
                           <label for="nombre_producto"><a href="Home.html"><%= producto.getDescCorta() %></a></label>
                       </div>
                       <div class="info_detalle_contenido">
                           <label id="subcategoria"><%= producto.getNombreSubcategoria()%></label><br>
                           <label id="existencia">Productos en existencia:</label><label id="existencia"><%= producto.getExistencia() %></label><br>
                       </div>
                      
                       <div class="prod_price_big"><label id="precio">$<%= producto.getPrecio() %></label></div>
                        <!--<div class="button1_pos"><button id="button1">Publicar</button> </div>-->
                        <div class="button10_pos"><form action="anuncio_edit.jsp?idProd=<%= producto.getId()%>" method ="post"><input type="submit" id="button1" value="Publicar"/></form> </div>
                        <div class="button2_pos"><form action="ProductoServlet?accion=editar&id=<%= producto.getId()%>" method ="post"><input type="submit" id="button2" value="Editar"/></form> </div>
                        <div class="button3_pos"><form action="ProductoServlet?accion=borrar&id=<%= producto.getId()%>" method ="post"><input type="submit" id="button3"value="Eliminar"/></form> </div>
                    </div>
            </div>
